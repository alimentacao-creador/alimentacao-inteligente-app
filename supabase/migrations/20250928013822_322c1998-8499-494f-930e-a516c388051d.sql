...
-- Enable RLS on notifications_queue table if not already enabled
alter table notifications_queue enable row level security;

-- Add RLS policy for notifications_queue if it doesn't exist
do $$
begin
  if not exists (
    select 1 from pg_policies 
    where tablename = 'notifications_queue' 
    and policyname = 'notifications_self'
  ) then
    execute 'create policy "notifications_self" on notifications_queue for all using (auth.uid()=user_id) with check (auth.uid()=user_id)';
  end if;
end $$;