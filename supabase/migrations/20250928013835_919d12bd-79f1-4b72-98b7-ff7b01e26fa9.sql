...
drop policy if exists "meals_self" on meal_images;
drop policy if exists "workouts_self" on workouts;
drop policy if exists "workoutex_self" on workout_exercises;
drop policy if exists "progress_self" on progress_monthly;
drop policy if exists "usage_self" on usage_limits;
drop policy if exists "events_self" on events_log;
drop policy if exists "notifications_self" on notifications_queue;
...
create policy "notifications_self" on notifications_queue for all using (auth.uid()=user_id) with check (auth.uid()=user_id);